    ...
    public class SQLFIX
    {
        public static void Sqlfix()
        {
            ...
            try
            {
                ...
                {
                    SqlCommand cmd = new SqlCommand(string.Format("SET IDENTITY_INSERT HighestNodeID ON"), conn);
                    conn.Open();
                    cmd.ExecuteNonQuery();
                    cmd = new SqlCommand(string.Format("INSERT into dbo.HighestNodeID (NodeID,Value)  VALUES ( 1 , 1)"), conn);
